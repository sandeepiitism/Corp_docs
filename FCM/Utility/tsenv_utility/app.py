...
def index():
    if request.method == 'POST':
        ...
        if uploaded_file.filename != '':
            ...
            ds_pH, yhat_pH, yhat_lower_pH, yhat_upper_pH = forecast_prophet(df, "pH")
            ds_conductivity, yhat_conductivity, yhat_lower_conductivity, yhat_upper_conductivity = forecast_prophet(df, "Cond ")
            ds_bod, yhat_bod, yhat_lower_bod, yhat_upper_bod = forecast_prophet(df, "BOD ")
            ds_cod, yhat_cod, yhat_lower_cod, yhat_upper_cod = forecast_prophet(df, "COD")
            ds_toc, yhat_toc, yhat_lower_toc, yhat_upper_toc = forecast_prophet(df, "TOC")
            ds_turb, yhat_turb, yhat_lower_turb, yhat_upper_turb = forecast_prophet(df, 'Turb ')
